...
func (s *Server) basicHandler() chi.Router {
	...
	r.Post("/reciepts", func(w http.ResponseWriter, r *http.Request) {
		reciept := new(models.Reciept)
		if err := json.NewDecoder(r.Body).Decode(reciept); err != nil {
			w.WriteHeader(http.StatusUnprocessableEntity)
			fmt.Fprintf(w, "Unknown err: %v", err)
			return
		}

		if err := s.store.Reciepts().Create(r.Context(), reciept); err != nil {
			w.WriteHeader(http.StatusInternalServerError)
			fmt.Fprintf(w, "DB err: %v", err)
			return
		}
	...
	r.Get("/reciepts", func(w http.ResponseWriter, r *http.Request) {
		reciepts, err := s.store.Reciepts().All(r.Context())
		if err != nil {
			w.WriteHeader(http.StatusInternalServerError)
			fmt.Fprintf(w, "DB err: %v", err)
			return
		...
		render.JSON(w, r, reciepts)
	})
	r.Get("/reciepts/{id}", func(w http.ResponseWriter, r *http.Request) {
		idStr := chi.URLParam(r, "id")
		id, err := strconv.Atoi(idStr)
		if err != nil {
			w.WriteHeader(http.StatusBadRequest)
		...
		reciept, err := s.store.Reciepts().ByID(r.Context(), id)
		if err != nil {
			w.WriteHeader(http.StatusInternalServerError)
			fmt.Fprintf(w, "DB err: %v", err)
			return
		...
		render.JSON(w, r, reciept)
	})
	r.Put("/reciepts", func(w http.ResponseWriter, r *http.Request) {
		reciept := new(models.Reciept)
		if err := json.NewDecoder(r.Body).Decode(reciept); err != nil {
			w.WriteHeader(http.StatusUnprocessableEntity)
			fmt.Fprintf(w, "Unknown err: %v", err)
		...
		err := validation.ValidateStruct(
			reciept,
			validation.Field(&reciept.ID, validation.Required),
			validation.Field(&reciept.Name, validation.Required),
		)
		if err != nil {
			w.WriteHeader(http.StatusUnprocessableEntity)
			fmt.Fprintf(w, "Unknown err: %v", err)
		...
		if err := s.store.Reciepts().Update(r.Context(), reciept); err != nil {
			w.WriteHeader(http.StatusInternalServerError)
			fmt.Fprintf(w, "DB err: %v", err)
			return
		}
	})
	r.Delete("/reciepts/{id}", func(w http.ResponseWriter, r *http.Request) {
		idStr := chi.URLParam(r, "id")
		id, err := strconv.Atoi(idStr)
		if err != nil {
			w.WriteHeader(http.StatusBadRequest)
		...
		if err := s.store.Reciepts().Delete(r.Context(), id); err != nil {
			w.WriteHeader(http.StatusInternalServerError)
			fmt.Fprintf(w, "DB err: %v", err)
			return
		}
